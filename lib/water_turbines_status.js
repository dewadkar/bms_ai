...
exports.status = function (callback) {
    ...
    writeFile('./output/water-turbines-data.csv', csvData, (err) => {
        if (err) {
            return callback(err);
        } else {
            csv().fromFile('./output/water-turbines-data.csv')
                .then(function (data) {
                    ...
                    writeFile('./output/failed-water-turbines.csv', failedCSVData, (err) => {
                        if (err) {
                            return callback(err)
                        } else {
                            var generator_json = [];
                            ...
                            writeFile('./output/water-turbines-scoring-data.csv', csvGeneratorData, (err) => {
                                if (err) {
                                    return callback(err)
                                } else {
...
exports.getTurbineDetails = function (id) {
    return new Promise(function (resolve, reject) {
        ...
        let file = './output/water-turbines-scoring-data.csv';
        csv().fromFile(file)
            .then(function (data) {
                if (data) {
                    for (var i = 0; i < data.length; i++) {
